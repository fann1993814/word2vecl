...
try:
    from .word2vecl_inner import (  # noqa: F401
        train_batch_sg,
        train_batch_cbow,
        MAX_WORDS_IN_BATCH,
        FAST_VERSION,
...
class Word2VecL(utils.SaveLoad):
    def __init__(
            self, sentences=None, corpus_file=None, word2labels=None, label_file=None, vector_size=100, alpha=0.025, beta=1.0, window=5, min_count=5,
        ...
        hs : {0, 1}, optional
            If 1, raise NotImplementedError.
        ...
        if word2labels is not None:
            self.word2labels = word2labels
        elif label_file is not None:
            if os.path.isfile(label_file):
                raise FileNotFoundError 
            self.word2labels = self.read_label_file(label_file)
        else:
            raise ValueError('labels or label_file cannot be null.')
    ...
    def prepare_vocab(
        ...
        if self.hs:
            ...
            raise NotImplementedError("hs mode not implemented")
        if self.negative:
    ...
    def add_labels(self):
        ...
        # add prefix for label terms
        for word in self.word2labels:
            self.word2labels[word] = [tmpl.format(label) for label in self.word2labels[word]]

        # counting label from words
        for word in self.word2labels:
            if word in self.wv.key_to_index:
                for label in self.word2labels[word]:
                    label2count[label] += self.wv.get_vecattr(word, 'count')
                    self.labels.add(label)

        if len(self.labels) == 0:
            raise Exception('No any word has corresponded label in the corpus with word2label.')

        # add dummy label for smoothing
        if len(self.labels) == 1:
            DUMMY = tmpl.format('Dummy')
            self.labels.add(DUMMY)
            label2count[DUMMY] = sum([label2count[l] for l in label2count]) // len(label2count)

        vocab_size = len(self.wv)

        # register label into wv
        for label in self.labels:
            self.wv.key_to_index[label] = len(self.wv)
            self.wv.index_to_key.append(label)

        # assgin freq
        for label in self.labels:
            label_idx = self.wv.key_to_index[label]
            self.wv.set_vecattr(label_idx, 'count', label2count[label])

        # build index arr for training
        self.word2label_idx = {}
        for word in self.word2labels:
            self.word2label_idx[word] = np.array([self.wv.key_to_index[label] for label in self.word2labels[word]], ndmin=0, dtype=np.uint32)
    ...
    def init_weights(self):
        ...
        if self.hs:
            raise NotImplementedError("hs mode doesn't supported")
        if self.negative:
    ...
    def train(
        ...
        for cur_epoch in range(self.epochs):
            ...
            if corpus_iterable is not None:
                ...
                raise TypeError('corpus_iterable cannot be null')
    ...
    def _check_corpus_sanity(self, corpus_iterable=None, corpus_file=None, passes=1):
        ...
        if corpus_iterable is None and corpus_file is not None:
            raise TypeError("Only support corpus_iterable for Word2VecL training")
