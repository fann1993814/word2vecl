...
def train_batch_sg(model, sentences, alpha, _work, compute_loss):
    ...
    for sent in sentences:
        ...
        for token in sent:
            ...
            # add label for each effective_words at this sentence
            if token in model.word2labels:
                c.indexes_label[effective_words] = <np.uint32_t *>np.PyArray_DATA(model.word2label_idx[token])
                c.indexes_label_size[effective_words] = model.word2label_idx[token].shape[0]
            else:
                c.indexes_label_size[effective_words] = 0
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                # label prediction
                if c.negative and c.indexes_label_size[i] != 0:
                    for n in range(c.indexes_label_size[i]):
                        c.next_random = w2v_fast_sentence_sg_neg(c.negative, c.cum_table, c.cum_table_len, c.syn0, c.syn1neg, c.size, c.indexes_label[i][n], c.indexes[i], c.alpha*c.beta, c.work, c.next_random, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss)
